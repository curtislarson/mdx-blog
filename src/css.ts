...
import presetWind from "https://esm.quack.id/@unocss/preset-wind@0.49.0?bundle&no-check";
// import presetDaisy from "https://esm.quack.id/unocss-preset-daisy@1.2.0";
import { PurgeCSS } from "https://esm.quack.id/purgecss@5.0.0";
import { presetUno } from "https://esm.quack.id/unocss@0.49.1";
// import transformDirectives from "https://esm.quack.id/v104/@unocss/transformer-directives@0.45.30";
...
const defaultUnoConfig: UserConfig = {
  presets: [presetWind(), presetTypeography()] as unknown as Preset[],
};

export type { UserConfig as UnoCSSConfig };

export function createCSSProcessor(config = defaultUnoConfig) {
  ...
  const daisyCSSPromise = fetch("https://esm.sh/daisyui@2.49.0/dist/full.css").then((r) => r.text());

  async function generate(body: string) {
    const [daisyCSS, { css }] = await Promise.all([daisyCSSPromise, uno.generate(body)]);
    return [unoResetCSS, daisyCSS, css];
  }

  async function purge(raw: string, rawCss: string[]) {
    return await purgecss.purge({ content: [{ extension: "html", raw }], css: rawCss.map((raw) => ({ raw })) });
  }
